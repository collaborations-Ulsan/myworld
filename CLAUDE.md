...
## DNA invariants — only the ones CODE enforces (pruned 2026-08-19)

Measured: of the original seven, five are enforced by code and two were prose only. Prose-only
rules are few-shot text that nothing checks, and leaving them on the list costs the credibility
of the five that are real. So they are down, not softened.

1. **Draft-first memory** — `memoryOS/memoryos/schema.py` (draft→accept lifecycle)
2. **Append-only audit** — every ledger is append-only jsonl folded by a pure function
   (`aios_factory.state`, `aios_decision_ledger`, `aios_mesh` events)
3. **Provenance chain** — `aios_capabilities.consumed_by`; a task cannot close on exit status
4. **Operator override always possible** — `scripts/aios_override.py`. Reason mandatory, every
   use logged; a gate overridden 3× for the same reason is reported as evidence the gate is wrong.
   Four gates are named NOT overridable (privacy, self-close, consumed-without-evidence,
   taking a control arm you authored the treatment for).
5. **Privacy boundary inviolable** — `_from_desktop`/`dain`/`minyoung`/secrets/raw exports.
   Enforced at the walker in `aios_graph_build.DENY_PARTS`, never at the writer.

REMOVED as unenforced prose: *recommendation-only (no auto-binding)* and *stop conditions named*.
Neither was ever checked by anything; the organs sat dead for seven days and neither rule noticed.
If either is wanted back, it comes back as code.
