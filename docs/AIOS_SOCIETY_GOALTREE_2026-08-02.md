# AIOS = AI Society — 목표 트리 (2026-08-02) · **2026-08-05 판정으로 종료**

> ## ⛔ 이 목표 트리는 닫혔다 (2026-08-05)
>
> **G5가 사회 층을 반증했다.** society 0.4062 vs solo+원장 **0.4688** (delta −6.25pp,
> McNemar p=0.856, 128/128 셀, 인프라·변조 0). 사전등록 kill rule 조건 1(C ≤ B) 발동.
> 판정과 적대 점검 전문: `docs/AIOS_G5_SOCIETY_PREREG_2026-08-03.md` Errata,
> `experiments/phase5g/G5_RESULTS.md`.
>
> **집행된 것:** 사회 층 해체 · **저장-대개편(단일 에이전트 + 아크 원장)으로 재수렴** ·
> 네트워크 N2~N4 **착수 안 함** · 컴퓨트 시스템 C1~C3 **착수 안 함** · QEL 흡수는 B1 프라이버시
> 설계만 남기고 보류 · `supersede`는 코드로 남기되 **가치 주장 철회**.
> **새 사회 메커니즘으로 재시도하지 않는다** — 그것이 §6에 미리 적힌 조건이다.
>
> **살아남은 것:** 아크 원장 자체가 **실험에서 가장 성적이 좋은 기제**였다
> (solo_ledger가 전 팔 1위, 무기록 대비 +9.38pp — 단 p=0.227로 **미확립이며 주장하지 않는다**).
> founder의 양자택일에 대한 데이터의 답은 **"저장 대개편"** 이다.
>
> 아래 내용은 판정 이전의 기록이며 **역사로 읽어야 한다**. G1~G3 구현·네트워크 테제·
> 프라이버시(B1) 분석은 계속 유효하지만, "사회가 더 낫다"는 전제는 죽었다.
...
## 이름 규율 (2026-08-03, C0 — 즉시 적용)

1. **"AI를 위한 OS"라고 부르지 않는다.** OS 유비는 결정성·싼 컨텍스트 스위치·idempotence를
   잘못 수입한다. 정식 명칭: **"작업을 위한 내용주소화 재계산 기계"**
   (`docs/AIOS_ZERO_TO_TEN_2026-08-03.md`). 조상은 Nix/Bazel 계열이지 Linux가 아니다.
2. **이름 충돌 방어.** 우리 `AIOS`는 arXiv:2403.16971의 "AIOS: LLM Agent Operating System"과
   **무관한 별개 프로젝트**다. 외부 문서·발표에서는 반드시 이 문장을 함께 적는다.
   (Orca가 GNOME 스크린리더와의 충돌을 문서 최상단에서 경고하는 규율을 흡수 —
   `docs/AIOS_ORCA_PEER_ANALYSIS_2026-08-03.md` §4.2)
3. **"우리가 처음"을 이 영역에서 쓰지 않는다.** 근접 peer 실재: Orca(사회·인수인계),
   PCAA·Proof-or-Stop(검증), OpenCrab(온톨로지). peer 존재 = 수요 검증.

## 프라이버시 재검토 — append-only가 만드는 새 위험 (2026-08-03, red-team 발)

적대 council: *"중앙 스케줄링과 provenance는 프롬프트·도구 인자·모델 출력·URL·파일명·에러
트레이스를 영구 2차 데이터 저장소로 만든다. 사용자 표시 출력에만 마스킹을 적용하면 로그·영수증·
트레이스·크래시 덤프에서 복구 가능하고, **append-only 보존은 철회·삭제 요구와 충돌한다.**"*

현재 상태 점검:
- 아크 이벤트는 **에이전트가 쓴 텍스트를 그대로 담는다**(`progress.text`). 원문 금지는 규칙일 뿐
  **강제되지 않는다** — HARD invariant("프라이버시 경계 불가침")와의 간극이 여기 있다.
- `supersede`는 **투영에서만** 감추고 로그에는 남긴다 — 이는 감사에는 옳지만 **삭제 요구를
  만족시키지 못한다.**

⟹ **미해결 과제로 등재**(G6 후보, 지금 짓지 않음): (a) 아크 기록에 들어가기 전 강제 스크럽,
(b) 삭제 가능한 층과 불변 층의 분리(내용은 삭제 가능·해시는 불변 = tombstone + 증명 유지),
