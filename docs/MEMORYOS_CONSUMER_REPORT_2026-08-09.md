...
## 9. ★흡수 측정 — 같은 코퍼스에서 memoryOS 2/6 vs BM25 40줄 6/6 (2026-08-13)

founder 지시: 프로바이더 문서를 전부 읽어 저장하고 memoryOS에 임베딩한 뒤,
**"실제로 이런 지식들이 너에게 필요할 때 흡수되는지"** 확인할 것.
저장 개수를 보고하는 것은 오늘 하루 종일 문서화한 실패(제공하고 안 쓰임)이므로,
**회수로 채점**했다.

### 프로토콜 (채점 전에 고정)

질문 6개는 전부 **오늘 실제로 부딪힌 것**이고, 답이 있는 페이지를 **프로브 실행 전에**
기록해뒀다. 코퍼스는 `docs/external/providers/` 212페이지(Claude 187 · Codex 15 ·
gemini-cli 9 · MCP 1), 전부 digest 주소로 저장.

채점: top-5 안에 정답 페이지가 있으면 HIT.

### 결과

| 회수기 | 코퍼스 | recall@5 |
|---|---|---|
| `aios_doc_harvest` BM25 (k1=1.5, b=0.75, ~40줄) | 같은 212페이지 | **6/6** |
| `memoryos search` (202파일 임포트 후) | 같은 212페이지 | **2/6** |

**임포트는 됐다** — 202파일, 1파일당 ~114 nodes / 141 edges. 문서는 안에 있다.
못 찾는 것은 **회수기**다.

미스한 4개: `.claude` 디렉터리 상태 위치 · 실행 중 세션에 이벤트 주입(channels) ·
도구 호출 가로채기(hooks) · 샌드박스 설정. 전부 BM25는 찾았다.

### 부수 관측

- 매 질의가 **항상 정확히 5건**을 반환한다(rc=0). 0건이 없으므로 "못 찾았다"가
  호출자에게 보이지 않는다 — §8의 조용한 강등과 같은 형태다.
- 질의당 **9~12초**. BM25는 전체 코퍼스에 대해 밀리초 단위다.

### 정직한 한계

memoryOS는 문서를 노드/엣지로 쪼개므로 단위가 다르다. 비교는 *"답이 있는 원본을
표면화하는가"* 수준에서 했고, 그게 흡수에서 실제로 중요한 것이다. 다만 이 프로브는
**노드 단위 정밀도**를 재지 않았고, 임베딩 백필(`--embed`)도 켜지 않았다 —
§8에서 그 경로가 이 기계에서 조용히 키워드로 강등된다는 것이 이미 측정됐기 때문이다.

### 요청

`aios_doc_harvest.py probe`를 그대로 회귀 테스트로 쓸 수 있다. 같은 6문항에서
**BM25 baseline(6/6)을 넘지 못하는 회수기는 개선이 아니다.** 지금은 넘지 못한다.
